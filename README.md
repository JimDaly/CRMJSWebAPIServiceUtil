...
experience with Visual Studio when writing client-side JavaScript to perform operations using the 
...
capabilities. The size of [jquery-3.1.0.min.js](https://code.jquery.com/jquery-3.1.0.min.js) 
is 85KB by comparison. Adding definitions for all entities and operations is not realistic
because the generated library would be huge. You should choose only those items
you will actually use in your project to mitigate the performance cost of the 
intial load of the library. Of course, once the library is cached in the user's browser
this 'cold-load' cost will be eliminated until you edit the library or the cache expires.

The library generated by this utillity should be ideal for complex HTML web resource or 
...
call from a form script or ribbon command, you should consider a more light-weight 
...
* TypeScript definition file for the generated JavaScript library. You can use the JavaScript library 
directly or the TypeScript definition in a TypeScript project.
* Option to generate a version of the library which includes adal.js and necessary changes to use the 
library in a Single Page Application (SPA) that uses OAuth with Cross-Origin Resource Sharing (CORS). 
...
* The following core functions are available for operations with entities:
    ...
    * **delete** (**deleteEntity** in TypeScript since it is more picky about keywords than JavaScript)
    ...
    * **disassociate** Dissassociates any kind of relationship (N:N,N:1,or 1:N)
...
* All entity classes inherit from a crmbaseentity class which provides the following methods:
    ...
    * Methods for each single-valued navigation property included in the library definition for an 
    entity. This allows for a IntelliSense discoverable way to set these properties since the @odata.bind annotation doesn't
...
* The constructor for all entity classes accept the following options to instantiate an entity:
...
* Metadata information such as entitysetname and the names of entity relationships so you don't 
need to look them up. All the relationships between any entities in your library definition are included
automatically.
...
* Option to save a definition of your library configuration so you can re-load it when you want to edit it.

[Go to Top](#in-this-readme)
## Limitations
The following are known limitations of the generated library:
* Provides no methods or helpers to execute batch operations.
* Does not support operations to query or edit entity metadata. See [Use the Web API with CRM metadata](https://msdn.microsoft.com/en-us/library/mt607833.aspx).
...
Including these comments in the generated JavaScript library is not enabled by default. This is not an issue when using the TypeScript definitition, so they are always included there.
...
1. Clone this repo and open the CRMJSWebAPIServiceUtil Visual Studio solution.
...
Many of the common properties you want to use with activites are in the  **activitypointer** entity.
So it is a good idea to explicitly choose **activitypointer**.

>**Remember**: The more entities you add to your library, the larger it will be and this can effect performance.

6. On the **Select Properties** tab select an entity and choose which properties of that entity you need to use in your
...
which are displayed as fields in the form. Some entities do not have forms, so this button will do nothing for those entities.
...
| **Sub Namespace**| Recommend you use a namespace that describes the sub-set of capablities in your library      | 
| **File Name**| The name of the files to generate. i.e.: Base.Sub.filename.js  and Base.Sub.filename.ts   | 
...
|**Choose Solution**| Choose from available unmanaged solutions in your deployment. Your choice will apply the appropriate customization prefix to the name of the web resource.|
...
> **Note**: You cannot load a library definition that doesn't match the environment used to create it. 
But the library definition is a JSON file, so you could potentially edit this file to make adjustments
so it will match a different environment. Frankly, I haven't tested this enought to know what kind of issues
might exist here.

[Go to Top](#in-this-readme)
## About this project
I am a writer who has taught himself to write code and I'm more comfortable writing Javascript than C#. This is my first WPF 
application. Please keep that in mind as I explain why I made this and how it works. Please feel free to let me know
where I have not followed conventions.

My goal here was simply to provide the design-time experience I want to have in Visual Studio when using the Web API with JavaScript.
...
as a sample in the CRM SDK. I wrote most of the content in the CRM SDK about the Web API and generated the reference documentation, so
I've had a relatively long time to get familar with the Web API since it was first introduced as a preview feature in CRM 2015. I've 
also benefited from access to the feature team which developed the API.
...
as much of that feedback as I could and they are some of the key design goals which were:
...
all of the entities and operations would be availble. This simply can't work with CRM and JavaScript.
...
pushed the boundaries of a sample. As a sample, we have no commitment to maintain it and we don't have the capabilites 
to incorporate community contributions. I will continue to maintain the Sdk.Soap.js sample as it was written, but I have no plans
...
turn it into something else altogether. I'm releasing this as an individual project because people might not be interested, or something 
better might come along. Time will tell.
