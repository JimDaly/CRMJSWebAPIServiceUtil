...
function startQueryDataSample() {
    ...
    ns.create(account)
        ...
        return ns.retrieve(account.getUri(), null, ["primarycontactid($select=" + contact.getColumnSet() + ")"], true);
    })
        ...
        return ns.retrieve(contact.getUri(), contact.getColumnSet(), null, true);
    })
        .then(function (entity) {
        var contact = new ns.contact(entity);
        console.log("Contact basic info:\n\tFullname: '%s'\n\tJob Title: '%s'\n\tAnnual Income: '%s' (unformatted)", contact.fullname, contact.jobtitle, contact.annualincome);
        ...
        .then(function (results) {
        output(results.value.map(function (x) { return new ns.contact(x); }), "Contacts top 5 results:", contactProperties);
        console.log("\n-- Result Count --");
        ...
        var str = "Account '%s' has the following primary contact person:\n\t" +
            "Fullname: '%s' \n\tJobtitle: '%s' \n\tAnnualincome: '%s'";
        console.log(str, account.name, contact.fullname, contact.jobtitle, contact.getFormattedValue("annualincome"));
        return ns.retrieve(contact.getUri(), contactProperties, ["account_primary_contact($select=" + accountProperties.join() + ")"], true);
    })
        .then(function (entity) {
        var contact = new ns.contact(entity);
        var label = "Contact '" + contact.fullname + "' is the primary contact for the following accounts:";
        output(contact.account_primary_contact.map(function (x) { return new ns.account(x); }), label, accountProperties);
        return ns.retrieve(account.getUri(), accountProperties, ["contact_customer_accounts($select=" + contactProperties + ")"], true);
    })
        .then(function (entity) {
        var account = new ns.account(entity);
        var label = "Account '" + account.name + "' has the following contact customers:";
        output(account.contact_customer_accounts.map(function (x) { return new ns.contact(x); }), label, contactProperties);
        ...
        return ns.retrieve(account.getUri(), accountProperties, expansions, true);
    })
        .then(function (entity) {
        var account = new ns.account(entity);
        var label = "Account '%s' has the following primary contact person:\n\t" +
            "Fullname: '%s' \n\tJobtitle: '%s' \n\tAnnualincome: '%s'";
        console.log(label, account.name, account.primarycontactid.fullname, account.primarycontactid.jobtitle, account.primarycontactid.annualincome);
  ...
  <entity name=\"contact\"> \
    ...
    <filter type=\"and\"> \
      <condition value=\"%(sample)%\" attribute=\"fullname\" operator=\"like\" /> \
        ...
        .then(function (results) {
        output(results.value.map(function (x) { return new ns.contact(x); }), "Contacts Fetched by fullname containing '(sample)' - page 3:", contactProperties);
        console.log("\n-- Saved Query -- ");
        ...
        .then(function (results) {
        output(results.value.map(function (x) { return new ns.account(x); }), "Saved Query (Active Accounts):", ["accountid", "name", "_primarycontactid_value"]);
        console.log("\n-- User Query -- ");
  ...
  <entity name=\"contact\"> \
    ...
    <filter type=\"and\"> \
      ...
      <condition value=\"55000\" attribute=\"annualincome\" operator=\"gt\" /> \
        ...
        .then(function (results) {
        output(results.value.map(function (x) { return new ns.contact(x); }), "Saved User Query:", contactProperties);
    })
        .catch(function (err) {
 ...
 */
function output(collection, label, properties) {
    console.log(label);
